...
def main():
    ...
    parser = argparse.ArgumentParser(prog='action', add_help=True)
    # Potential improvement: Fetch the valid choices for each argument from some resource, and fill them in below. `sdk_resource` and `build_list` have hard-coded choices.
    ...
    try:
        ...
        sys.exit(2) # We want a non-zero exit status even for help messages.
    ...
    # Set arguments as output: As JSON, and each item individually
